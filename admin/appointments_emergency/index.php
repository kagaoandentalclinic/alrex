        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
					<?php 
						$i = 1;
						$qry = $conn->query("SELECT * from `appointment_list` where status='0' order by unix_timestamp(`date_created`) asc ");
						while($row = $qry->fetch_assoc()):
					?>
						<tr>
							<td class="text-center"><?php echo $i++; ?></td>
							<td class=""><b><?php echo date("M d, Y",strtotime($row['schedule'])) ?></b></td>
									<td class=""><b><?php echo date("h:i ", strtotime($row['time'])) ?></b></td>
