...
if($_settings->userdata('id')){
    $qry = $conn->query("SELECT a.*,c.name as pet_type from `appointment_list` a inner join category_list c on a.category_id = c.id where a.clientid = '".$_settings->userdata('id')."'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k)){
    ...
    echo "<script>alert('Please Make an Appointment to view this page!'); location.replace('./?page=setappointment');</script>";
    }
}
else{
    echo "<script>alert('Appointment Request ID is required'); location.replace('./?page=setappointment');</script>";
}
$service = "";
...
$usercol = "";
$users = $conn->query("SELECT * FROM `users` where id in ({$_settings->userdata('id')}) ");
...
<b>
Alrex Driving School<BR>Remarks Record</BR>
        ...
        <div class="container-fluid">
            ...
            <table class="table table-hover table-striped table-bordered">
                ...
                <tbody>
                    <?php 
                        $i = 1;
                        $qry = $conn->query("SELECT * from `student_remarks` where userid='".$_settings->userdata('id')."'  ");
                        while($row = $qry->fetch_assoc()):
                    ?>
                        <tr>
                            <td class="text-center"><?php echo $i++; ?></td>
                             <td class="">  <?php echo ($row['date_created']) ?></td>
                                <td class="">  <?php echo ($row['refnum']) ?></td>
                            <td class=""><?php echo ($row['instructorid']) ?></td>
...
<br><br><br><center>
<b><u>_____________________________________________</u></b><br>
Administration
