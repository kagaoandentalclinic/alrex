...
if (!isset($_GET['page']) && in_array($_settings->userdata('type'), [3, 4, 5])) {
    header('Location: ' . base_url . 'admin/?page=setappointment');
    exit;
}
?>
 <!DOCTYPE html>
...
<?php require_once('inc/header.php') ?>
  <body class="layout-fixed layout-navbar-fixed" data-new-gr-c-s-check-loaded="14.991.0" data-gr-ext-installed="" style="height: auto;">
    <div class="wrapper">
     ...
     <?php if($_settings->chk_flashdata('success')): ?>
      ...
      <?php endif;?>    
     <?php
        $raw_page = isset($_GET['page']) ? $_GET['page'] : 'home';
        // Sanitize: strip any directory traversal characters, allow only word chars, hyphens, and forward slash
        $page = preg_replace('/[^a-zA-Z0-9_\-\/]/', '', $raw_page);
        // Prevent traversal: block any attempt to go up directories
