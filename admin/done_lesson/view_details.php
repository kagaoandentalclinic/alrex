...
<style>
    ...
    @media print {
        ...
        #remarks-print-area, #remarks-print-area * { visibility: visible; }
        #remarks-print-area { position: absolute; top: 0; left: 0; width: 100%; }
    }
            ...
            <div class="container-fluid">
                ...
                <div class="rounded-0 text-center mt-6 bg-gradient-dark">
                    <br>
                        ...
                        <a class="btn btn-light border  btn-md" href="./?page=done_lesson" ><i class="fa fa-angle-left"></i> Back to List</a>
                        <hr>
...
<div id="remarks-print-area">
<div class="d-flex justify-content-between align-items-center mb-2">
    <h2 class="mb-0">Remarks History</h2>
    <button class="btn btn-primary btn-sm" onclick="window.print()"><i class="fa fa-print"></i> Print Remarks</button>
</div>
<div class="show-print text-center mb-3">
    <h4>ALREX Driving School</h4>
    <p><strong>Student:</strong> <?= htmlspecialchars($requestor) ?> &nbsp;|&nbsp; <strong>Ref #:</strong> <?= htmlspecialchars($code) ?></p>
    <hr>
</div>
...
<table class="table table-hover table-striped table-bordered">
                ...
                <tbody>
                    <?php
                        $i = 1;
                        $qry = $conn->query("SELECT * FROM `student_remarks` WHERE refnum = '{$code}' ORDER BY unix_timestamp(`date_created`) DESC");
                        while($row = $qry->fetch_assoc()):
                            $overall = strtoupper($row['overall_status'] ?? '');
                            $badge = ($overall === 'COMPLETED') ? 'badge-success' : (($overall === 'FAILED') ? 'badge-danger' : 'badge-secondary');
                    ?>
                        <tr>
                            ...
                            <td class="text-center"><?php echo $row['written_result'] ?? '—'; ?></td>
                            <td class="text-center"><?php echo $row['practical_result'] ?? '—'; ?></td>
                            <td class="text-center">
                                <?php if ($overall): ?>
                                    <span class="badge <?= $badge ?>"><?= $overall ?></span>
                                <?php else: echo '—'; endif; ?>
                            </td>
                        </tr>
                    <?php endwhile; ?>
