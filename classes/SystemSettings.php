...
class SystemSettings extends DBConnection{
	...
	function update_settings_info(){
		...
		foreach($_POST['content'] as $k => $v){
			// Sanitize key: allow only simple alphanumeric/underscore names, no path traversal
			$safe_key = preg_replace('/[^a-zA-Z0-9_]/', '', $k);
			if(!empty($safe_key)){
				file_put_contents(base_app . $safe_key . ".html", $v);
			}
		}
